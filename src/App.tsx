...
export default function App() {
  ...
  useEffect(() => {
    ...
    // Announcement banner listener (optional)
    const unsubsBanner = onSnapshot(
      doc(db, 'settings', 'announcement'),
      (snap) => {
        if (snap.exists()) {
          setAnnouncementBanner(snap.data().content || '');
        } else {
          setAnnouncementBanner('');
        }
      },
      (_error) => {
        // Silently catch permission-denied or missing document errors for announcement banner
        setAnnouncementBanner('');
      }
    );
  ...
  const handleRegister = async (
    ...
    try {
      ...
      const adminDocRef = doc(db, 'users', telegramId);
      ...
      if (adminDocSnap.exists()) {
        ...
        showToast('Account linked with admin registered profile!', 'success');
      } else {
