...
export default function App() {
  ...
  const handleLogin = async (email: string, pass: string): Promise<boolean> => {
    try {
      ...
      if (qSnap.empty && !isOwner) {
        showToast('Account not found! You have not registered yet. Please Sign Up first.', 'error');
        return false;
      }
  ...
  const handleRegister = async (
    ...
    try {
      ...
      // DO NOT AUTO-LOGIN! Immediately sign out so user must log in explicitly.
      await signOut(auth);
